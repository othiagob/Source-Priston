...
bool CZipArchive::AddNewFile(CZipAddNewFileInfo& info, LPCTSTR comment )
{
	...
	std::string strFileName = info.m_szFileNameInZip;
	std::transform( strFileName.begin(), strFileName.end(), strFileName.begin(), ::tolower );
	info.m_szFileNameInZip = strFileName.c_str();

	//info.m_szFileNameInZip = info.m_szFileNameInZip.MakeLower();
