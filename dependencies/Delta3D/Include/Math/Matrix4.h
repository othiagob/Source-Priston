...
public:
	union
	{
		...
		int i[16];
		int m[4][4]; //!< Matrix  array format
	};

	//! Multiply operator with a Matrix4.
	Matrix4 operator *( const Matrix4Int& other ) const;
};
...
public:
	...
	//! Assign operator with a Matrix4.
	Matrix4& operator =( const Matrix4Int& other );
	...
	{
		__m128 m0 = _mm_loadu_ps( &_11 );
		__m128 m1 = _mm_loadu_ps( &_21 );
		__m128 m2 = _mm_loadu_ps( &_31 );
		__m128 m3 = _mm_loadu_ps( &_41 );
		_MM_TRANSPOSE4_PS( m0, m1, m2, m3 );      // NOLINT(modernize-use-bool-literals)
		Matrix4 out;
		_mm_storeu_ps( &out._11, m0 );
		_mm_storeu_ps( &out._21, m1 );
		_mm_storeu_ps( &out._31, m2 );
		_mm_storeu_ps( &out._41, m3 );
		return out;
	}
