...
ISAOCHECK::ISAOCHECK()
{
	...
	// Inicializa hostNameDraco com o valor do arquivo Connect.ini
	std::string clanConfig = LerClanConnect();
	if (!clanConfig.empty()) {
		strcpy_s(hostNameDraco, sizeof(hostNameDraco), clanConfig.c_str());
	} else {
		// Valor padrão caso não consiga ler do arquivo
		strcpy_s(hostNameDraco, sizeof(hostNameDraco), "189.46.228.170:30602");
	}
}
