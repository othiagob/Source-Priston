...
struct NewChatWindow
{
	...
	void Draw(const char* title, bool* p_open)
	{
		...
		// Se a checkbox de nùo mover estiver selecionada, trava o chat na posiùùo
		if (isMovingEnabled) {
			window_flags |= ImGuiWindowFlags_NoMove;
			window_flags |= ImGuiWindowFlags_NoResize;
		}

		// Sobe o chat se alguma interface for aberta
		...
		// Coloca o chat na posiùùo inicial        
		if (flagResize) {
		...
		ImGui::SetNextWindowBgAlpha(0.70f); // Transparùncia do chat
		...
		if (!isMovingEnabled)
		{
			...
			{
				if (ImGui::Selectable(u8"Posiùùo Original")) flagResize = true;
		...
		{	
			...
			if (chatMessage[0])
			{
				...
				// Adiciona a mensagem ao vetor de histùrico
