...
int ItemIndex1[3] = { 0,0,0 };	//?????? ???? ????? ?ùù??? ???? 
int ItemIndex2[3] = { 0,0,0 };

int ChangeJobButonCheck2 = 0; //???????? ??? ????? ?ùù?(???? ???? ??????? ????!!)
...
char* sinSoundWav[] = {
	...
	"Image\\SinImage\\Sound\\Armor.wav",			//9 (?ùù?)
	...
	"Image\\SinImage\\Sound\\sheltom-failure.wav",  //23 (?????, ????ùù ???ùù? ????)
	"Image\\SinImage\\Sound\\Armor-w.wav",          //24 (????)
	"Image\\SinImage\\Sound\\drink2.wav",           //25 (??ùù? ????)
	0
};
...
int sinSetShopItem(sITEMINFO* pItem, int ItemCount, int ItemKind)
{
	...
	cInterFace.CheckAllBox(SIN_SHOP);  //?????? ?ùù????? ??????? 
	ShopArrowPosi = 0; //???? ???? ???? 
	return TRUE;
}
...
int sinSetQuestItem(sITEMINFO* sItemInfo)
{
	...
	if (LoadItemImage(sItemInfo, &sinTempItem)) {
		...
		if (!cInvenTory.AutoSetInvenItem(&sinTempItem, 1)) { //????????????? ????? ??????? ??????????
			...
			cInvenTory.ChangeABItem(sArrowPosi); // 2???? ?????? ?ùù? ??? 
			if (!cInvenTory.AutoSetInvenItem(&sinTempItem, 1)) {
				...
				if (sItemInfo->PotionCount <= 1) {
					for (int k = 0; k < MAX_ITEM; k++) {
						if ((sinPZ1 | sin00) == sItem[k].CODE) {
							...
							sItem[j].lpTempItem = LoadDibSurfaceOffscreen(szFilePath); //??????? ?????ùù????
						}
					}
				}
			}
...
void ShowInterFace()
{
	...
	while (1) {
		...
		// Baù ImGui: nùo deixar o HUD de pedra (mix/aging/trade) deslizar por cima.
		if (cWareHouse.OpenFlag &&
			(cnt == SIN_TRADE || cnt == SIN_WAREHOUSE || cnt == SIN_CRAFTITEM || cnt == SIN_AGING ||
				cnt == SIN_SMELTING || cnt == SIN_MANUFACTURE || cnt == SIN_MIXTURE_RESET)) {
		...
		if (((sinFlag*)szKindInter[cnt])->OpenFlag > 0) {
			...
			if (cnt == 1) { //?ùù??????? (??? ????ùù?)
/*				CrashItemIndex[0] = 0; //?ùù????? ?????? ?????????
				CrashItemIndex[1] = 0;
				AutoSetItemIndex = 0;       //?????? ???????? ??????? ????? ???????? ?ùù???
				SelectInvenItemIndex=0;		//????? ???????? ?ùù???
*/
				SelectPotionIndex = 0;
...
int sinUsePotion() //????? ?????????? ?????? ?????? 
{
	...
	if (PotionKind) {
		if (pUsePotion->sItemInfo.PotionCount >= 2) { //?????? ???? ???ùù? 
			...
			*/
			...
			pUsePotion->Flag = 0;
			sInven[pUsePotion->ItemPosition - 1].ItemIndex = 0; //?????? ??ùù? 
			if (PotionKind != 5) {
				/*
					if(( pUsePotion->sItemInfo.CODE & sinITEM_MASK3 ) == sin03 ||
						( pUsePotion->sItemInfo.CODE & sinITEM_MASK3 ) == sin04 ){
...
//			else cTrade.OpenFlag = 1;
		...
		if (sinGetKeyClick('I')) {    //????ùù ????    
			...
			sinCheckAgingLevel(SIN_AGING_DEFENSE_BOOTS, false);   // ????? - ????ùù ?????? ???(????)
			sinCheckAgingLevel(SIN_AGING_DEFENSE_GLOVES, false);  // ????? - ????ùù ?????? ???(??)
			sinCheckAgingLevel(SIN_AGING_DEFENSE_ARMLETS, false); // ????? - ????ùù ?????? ???(???)
			...
			if (SelectInvenItemIndex) {
				...
				*/
				...
				ReformMixItem(&sCraftItem.CraftItem[0].sItemInfo ); //????ùù ??????
				cCraftItem.ReFormCraftItem();   //?ùù??? ?????? ????
				*/
				...
				ReformMixItem( &cInvenTory.InvenItem[SelectInvenItemIndex-1].sItemInfo  ); //?????? (???????? ?ùù????? ??????????? ...)
...
//	short   ItemAgingNum[2];	//0?? ????ùù+?? 1?? ????ùù?? ?????? ?????
...
int sinSetAgingItemIncreState(sITEMINFO* pItem)
{
	...
	if (pItem->Level == 100 || pItem->Level == 102 || pItem->Level == 105 ||	// pluto 102???? ???? ???? ???? ????? // ?? - 105?? ?????? ????
		cAging.AginStoneKind == 3 || cAging.AginStoneKind == 4 || // ????? - ???? ???? ????, ???? ????ùù ???? ??????? ????ùù ?????? ?????? ?????? ?????? ????? ??ùù?.
		cAging.AginStoneKind == 5) // ?? - ???? ????ùù ???? 1.5
	{
		pItem->Level += 0;
	}
	else
...
int sinAgingItemInit(sITEMINFO* pItem, int Down)
{
	...
	pItem->ItemAgingNum[0] -= DownNum;	//???? ?????? ??????? (?????? ??ùù??)
/*
...
int sinCheckAgingLevel(int Kind, bool bBillingUse)
{
	...
	case  SIN_AGING_BLOCK:
		...
		// ????? - ????ùù ?????? ??????
	case SIN_AGING_DEFENSE_ARMOR: // ????, ?ùù?
		// ???? , ?ùù? 	
		if (cInvenTory.InvenItem[sInven[2].ItemIndex - 1].Flag ||
			cInvenTory.InvenItem[sInven[2].ItemIndex - 1].sItemInfo.ItemAgingNum[1]) {
			if ((cInvenTory.InvenItem[sInven[2].ItemIndex - 1].CODE & sinITEM_MASK2) == sinDA1 ||
	...
	if (pWingItem->Result == TRUE) {
		...
		if (pWingItem->DesCraftItem.sItemInfo.CODE) {
			if (LoadItemImage(&pWingItem->DesCraftItem.sItemInfo, &sinTempItem)) {
				...
				cInvenTory.AutoSetInvenItem(&sinTempItem, 1 + pWingItem->DocIndex); //?ùù????? 1????????? ????? ???????
				cInvenTory.ReFormInvenItem();
				ResetPotion();
				cInvenTory.ReFormPotionNum();	//???? ?????? ?????
			}
...
int sinCloseInterFace()
{
	...
	if (cInvenTory.OpenFlag) cInvenTory.OpenFlag = SIN_CLOSE; //?ùù????? ?????????? ??????? 		 
	if (cCharShop.OpenFlag) cCharShop.OpenFlag = SIN_CLOSE; //?????? ?????????? ??????? 		 
	if (cCharShop.OpenFlag) {
		cCharShop.OpenFlag = 0; //?????? ??ùù?
...
int sinRecvForceOrb(sITEMINFO* pItemInfo, int Count)
{
	...
	ReformCharForm();//?????? 
	sinbaram_Stop = 0; //?????? ?????????????
	MixItemNoCopyFlag = 0; //??????? ?ùù? ????
	cCraftItem.OpenFlag = 0; //??? ??ùù? 
