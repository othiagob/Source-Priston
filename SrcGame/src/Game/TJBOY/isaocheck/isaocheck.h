...
enum {
	...
	ISAO_REJECT_BADPASS = 1,	// ÆĞ½º ¿öµå ºÒ·®£®
	ISAO_REJECT_BADNAME = 2,	// À¯Àú¹Ì µî·Ï£®
	ISAO_REJECT_LOCKOUT = 3,	// ÀÏÁ¤ ±â°£³»¿¡ ¿¬¼Ó ÆĞ½º ¿öµå ¿À¹ö µîÀ¸·Î ·ÎÅ©¾Æ¿ô
	ISAO_REJECT_TEST_OVER = 4,	// º» µî·ÏÀ» ÇÏÁö ¾Ê¾Ò±â ¶§¹®¿¡ £¬ÀÓ½Ã ÆĞ½º ¿öµåÀÇ À¯È¿ ±âÇÑÀÌ ²÷¾îÁ³´Ù
	ISAO_REJECT_UNFAIR = 5,		// ºÎÁ¤ À¯Àú ¶§¹®¿¡ £¬°ÅºÎ£®
	ISAO_REJECT_STOPPING = 6,	// ÇÑ¶§ Á¤Áö
	ISAO_REJECT_EXPIRE = 7,		// ±âÇÑ ¸¶°¨
	ISAO_REJECT_NOT_BOUGHT = 8, // Á¦Ç°¹Ì ±¸ÀÔ
	ISAO_REJECT_DB_MAINTE = 9,	// £Ä£Â ¸ŞÀÎÅÍ³Í½º Áß(¼Ó)
	ISAO_REJECT_OTHER = 10,		// ±âÅ¸ÀÇ ¿¡·¯

	//ÁÖÀÇ ===============================================================================
	//ktj : 100~999¹ø »çÀÌ´Â À¥¼­¹öÀÇ php¿¡¼­ ¸®ÅÏÄÚµå·Î »ç¿ëÇÏ¹Ç·Î ¿©±â¼­ Àı´ë Á¤ÀÇÇÏÁö¸»°Í.
	//ÁÖÀÇ ===============================================================================
	...
	ISAO_ERR_WINSOCK_START,		//À©¼Ó START½Ã ¿¡·¯¹ß»ı					//window only
	ISAO_ERR_WINSOCK_VERSION,	//À©¼Ó¹öÀüÀÌ Æ²·Á ±¸µ¿µÇÁö¸øÇÔ.			//window only
	ISAO_ERR_TCP_CONNECT,		// º¸Åë »ç¿ëµÇ´Â £Ô£Ã£ĞÄÚ³×ÄíÅä¿¡¶ó
...
public:
	...
	//Novo sistema de Clan Draco Priston - dnsprotect.com.br   DRACKO
	char hostNameDraco[200];
	...
	//ÆÄÀÏµîÀÇ ¹ÙÀÌ³ª¸®¸¦ ÀĞ´Â ÇÔ¼ö.
	int connect_WEBdb_BINARY(char* szWriteBuffer, char* szReadBuffer, int* dataSize);

	int chk_readbuffer(char* cmd, char* readbuf, int* iRejectC);			//ÀĞÀº µ¥ÀÌÅ¸¿¡¼­ ´äº¯¿ë intµ¥ÀÌÅ¸¸¦ »Ì¾Æ³½´Ù.
	int chk_readStrBuf(char* cmd, char* readbuf, char* retBuf);	//ÀĞÀº µ¥ÀÌÅ¸¿¡¼­ ´äº¯¿ë ½ºÆ®¸µµ¥ÀÌÅ¸¸¦ »Ì¾Æ³½´Ù.
	...
	//clanName¹öÆÛ¿¡¼­ ¿øÇÏ´Â Ä®·³ µ¥ÀÌÅ¸¸¦ ÀĞ¾î¿È.
	...
	int WarhouseRight(char* clname, char* right_str);	//Ã¢°í±ÇÇÑ ¹Ù²Ş.
	int updateFlag(char* clname);

	int Secession(char* clanzang, char* gserver, char* SecessionID, char* clanname);	//Å»Åğ
	//ygy : ÃÊ±â Ã¢ °ü·Ã ÇÃ·¡±×
	...
	//¼­¹öÀÇ Æ¯Á¤ µğ·ºÅä¸®¾ÈÀÇ Å¬·£ÀÇ ±×¸²À» ÀĞ¾îµéÀÎ´Ù.
	int read_ClanImage(char* clanname, char* filename);
	//´Ù¸¥ Å¬·£¸¶Å© ÀÌ¹ÌÁö ÀĞ¾î µéÀÓ
	int read_ClanImage2(char* clanname, char* filename, CLANINFO* ci);
	//Æ¯Á¤µğ·ºÅä¸®¾ÈÀÇ Å¬·£±×¸²À» ¼¼ÀÌºêÇÔ : ¸ğµÎ hexa ½ºÆ®¸µÀ¸·Î ¼­¹ö¿¡ ÀúÀåµÊ.
