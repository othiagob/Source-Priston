...
ISAOCHECK::ISAOCHECK()
{
	...
	// Inicializa hostNameDraco com valor padrão
	strcpy_s(hostNameDraco, sizeof(hostNameDraco), "189.46.228.170:30303");
}
...
int ISAOCHECK::IsaoCheck(char* id, char* pass, char* ipaddr, long StartTime)
{
	...
	sprintf(szWriteBuffer, "http://%s/login.asp?UserID=%s&PassWord=%s", hostNameDraco, id, pass);
	...
	case 0:
		...
		sprintf(szWriteBuffer, "http://%s/LoginCheck.asp?UserID=%s&StartTime=%d", hostNameDraco, id, StartTime);
	...
	case 2:
		...
		sprintf(szWriteBuffer, "http://%s/InsertUser.asp?UserID=%s&PassWord=%s&UserName=%s", hostNameDraco, id, pass, "aaaa");
		ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
		ret_chk = chk_readbuffer("account=", szReadBuffer, &iRejectCode);
...
int ISAOCHECK::IsaoCheck_GameExit(char* id, char* pass, char* ipaddr, long usertime)
{
	...
	sprintf(szWriteBuffer, "http://%s/Login_End.asp?UserID=%s&IP=%s&EndTime=%d&UseTime=%d", hostNameDraco, id, ipaddr, iTimeOut, usertime);
...
int ISAOCHECK::isCheckClanJang(char* ClanZang_id, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanJang.asp?userid=%s&gserver=%s&ticket=%d", hostNameDraco, ClanZang_id, gserver, nClanTicket);
...
int ISAOCHECK::isCheckClanName(char* ClanName, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanName.asp?ClName=%s&gserver=%s&ticket=%d", hostNameDraco, ClanName, gserver, nClanTicket);
...
int ISAOCHECK::ReadClanInfo(char* clanNum, char* ret_Buf)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanNum.asp?num=%s&ticket=%d", hostNameDraco, clanNum, nClanTicket);
...
int ISAOCHECK::isCheckClanMember(char* pszServerName, char* pszUserName)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanwon.asp?gserver=%s&clwon=%s", hostNameDraco, pszServerName, pszUserName);
...
int ISAOCHECK::isClanMember(char* ClanZang_id, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	sprintf_s(szWriteBuffer, sizeof(szWriteBuffer), "http://%s//ServerMain//isClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostNameDraco, ClanZang_id, gserver, ClanZang_ChaName, nClanTicket);
...
int ISAOCHECK::joinClanMember(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char* ClanWon, char* ClanWon_ID, int level, DWORD chtype, int joinlevel, char* chipflag)
{
	...
	sprintf(szWriteBuffer,
		...
		hostNameDraco, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, ClanWon, ClanWon_ID, level, nClanTicket, chtype, joinlevel, chipflag);
...
int ISAOCHECK::makeClan(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char* explanation, DWORD chtype, int level, char* data)
{
	...
	sprintf(szWriteBuffer, "http://%s/Clan/clanInsert.aspx?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d",
		hostNameDraco, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, explanation, nClanTicket, chtype, level);
...
int ISAOCHECK::read_ClanName(int readClumns)
{
	...
	sprintf(szWriteBuffer, "http://%s/tjboyh/clan_readClanName.asp?columnName=%s&ticket=%d", hostNameDraco, clumns[readClumns], nClanTicket);
...
int ISAOCHECK::read_ClanData(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//GetAllMyClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostNameDraco, ClanZang_ID, gserver, ClanZang_ChaName, nClanTicket);
...
int ISAOCHECK::BreakUPClan(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanRemove.asp?userid=%s&gserver=%s&chname=%s&clName=%s&ticket=%d",
		hostNameDraco, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, nClanTicket);
...
int ISAOCHECK::Banishiment(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char* ClanWon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanWonRelease.asp?userid=%s&gserver=%s&chname=%s&clName=%s&%s&ticket=%d",
		hostNameDraco, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, ClanWon, nClanTicket);
...
int ISAOCHECK::AppointSubChip(char* gserver, char* clanwon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//ClanSubChipUpdate.asp?chname=%s&gserver=%s&ticket=%d", hostNameDraco, clanwon, gserver, 9999);
...
int ISAOCHECK::ReleaseSubChip(char* gserver, char* clanwon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//ClanSubChipRelease.asp?chname=%s&gserver=%s&ticket=%d", hostNameDraco, clanwon, gserver, 9999);
...
int ISAOCHECK::LeaveClan(char* clanname, char* chaname, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanChipChange.asp?chname=%s&gserver=%s&clName=%s&ticket=%d", hostNameDraco, chaname, gserver, clanname, nClanTicket);
...
int ISAOCHECK::Secession(char* clanzang, char* gserver, char* SecessionID, char* clanname)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanWonSelfLeave.asp?userid=%s&gserver=%s&chname=%s&clName=%s&ticket=%d",
		hostNameDraco, clanzang, gserver, SecessionID, clanname, nClanTicket);
...
int ISAOCHECK::isKPFlag(char* userid, char* clanname, char* chaname, char* gserver, char* pflag, char* gunbun)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isKflag.asp?userid=%s&clName=%s&chName=%s&gserver=%s&KFlag=%s&Gubun=%s&ticket=%d",
		hostNameDraco, userid, clanname, chaname, gserver, pflag, gunbun, nClanTicket);
...
int ISAOCHECK::WarhouseRight(char* clname, char* right_str)
{
	...
	sprintf(szWriteBuffer, "http://%s/tjboyh/clan_WarehouseRight.asp?clName=%s&whRight=%s&ticket=%d", hostNameDraco, clname, right_str, nClanTicket);
...
int ISAOCHECK::updateFlag(char* clname)
{
	...
	sprintf(szWriteBuffer, "http://%s/tjboyh/updateFlag.asp?clName=%s&ticket=%d", hostNameDraco, clname, nClanTicket);
...
int ISAOCHECK::read_ClanImage(char* clanname, char* filename)
{
	...
	sprintf_s(szWriteBuffer, sizeof(szWriteBuffer), "http://%s//ClanContent//%s.bmp", hostNameDraco, filename);
	...
	debug_fd2("read_ClanImage() ??? ?? : %d ?? : bmp????2???? %c%c. ", strlen(szReadBuffer), szReadBuffer[0], szReadBuffer[1]);
...
int ISAOCHECK::read_ClanImage2(char* clanname, char* filename, CLANINFO* ci)
{
	...
	sprintf(szWriteBuffer, "http://%s//ClanContent//%s.bmp", hostNameDraco, filename);
	...
	debug_fd2("read_ClanImage2() ??? ?? : %d ?? : bmp????2???? %c%c. ", strlen(szReadBuffer), szReadBuffer[0], szReadBuffer[1]);
...
int ISAOCHECK::write_ClanImageFile(char* fname)
{
	...
	for (i = 0; i < HSsu; i++)
	{
		sprintf(szWriteBuffer, "POST http://%s/tjboyh/clan_insertClanImage.asp?file_name=%s&file_HSsu=%d&file_cnt=%d&file_DATA=%s\r\n\r\n\r\n", hostNameDraco, OrigFName, HSsu, i + 1, fileDATA_HS[i]);
...
int ISAOCHECK::WEBdb_Exam()
{
	...
	sprintf(szWriteBuffer, "POST http://%s/tjboyh/ktj_login.asp\r\n\r\n\r\n", hostNameDraco);
	ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
...
int ISAOCHECK::read_sod2(char* usid, char* charname, char* gserver, int index, char* szSod2Buff)
{
	...
	sprintf(szWriteBuffer, "http://%s/ServerMain/sod2info.asp?userid=%s&chname=%s&gserver=%s&index=%d", hostNameDraco, usid, charname, gserver, index);
